          ...
          {filteredProducts.map((product, index) => (
            ...
            >
              ...
              <div className="p-5 bg-white">
                <span className="text-xs text-[#aea4a4] tracking-wide uppercase">{t(`resources.categories.${product.category.toLowerCase()}`)}</span>
                <h3 className="font-serif text-xl text-black mt-1">{t(`resources.items.${product.name.toLowerCase().replace(/\s+/g, '').replace(/-/g, '')}`)}</h3>
              </div>
            </div>
          ))}
        </div>
